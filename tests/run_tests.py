...
class Colors:
    ...
    def print(text, color=NC, end="\n"):
        ...
        try:
            ...
            if sys.stdout.isatty() and platform.system() != "Windows":
                safe_text = f"{color}{text}{Colors.NC}".encode(
...
def run_quark_test(neutron_bin, root_dir):
    ...
    try:
        ...
        result = subprocess.run(
            [neutron_bin, "main.nt"], cwd=quark_dir, capture_output=True, text=True
        )
...
def compare_outputs(expected, actual, test_name):
    ...
    expected_normalized = "\n".join(
        line.strip() for line in expected.strip().splitlines()
    )
    ...
    for i, (exp_line, act_line) in enumerate(zip(expected_lines, actual_lines)):
        if exp_line != act_line:
            ...
            mismatches.append({"line": i + 1, "expected": exp_line, "actual": act_line})
...
def run_box_test(neutron_bin, root_dir):
    ...
    try:
        ...
        result = subprocess.run(
            [box_bin, "install", "base64"], capture_output=True, text=True
        )
        if result.returncode != 0:
            print(f"  ", end="")
            Colors.print("[FAIL]", Colors.RED, end="")
        ...
        result = subprocess.run(
            [neutron_bin, test_file], capture_output=True, text=True
        )
                    ...
                    try:
                        ...
                        if run_result.returncode == 0:
                            ...
                            if run_result.stderr.strip():
                                print(f"    STDERR: {run_result.stderr.strip()[:5000]}")
                        ...
                        Colors.print(f"  [INFO]", Colors.YELLOW, end="")
                        print(
                            f" {test_name} (AOT binary encoding issue, using interpreter)"
                        )
