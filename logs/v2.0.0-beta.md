...
Neutron supports **static variables** - immutable constants that cannot be reassigned after initialization.
...
- **Immutable**: Cannot be reassigned once initialized
- **Global Scope**: Accessible throughout the entire program
- **Type Annotations**: Can be combined with type annotations for extra safety
...
// MAX_USERS = 200;  // ❌ Runtime error: Cannot reassign static variable
```
...
├── .box/              # Box modules (optional)
│   └── modules/
└── build/             # Build output
    └── myproject      # Standalone executable
```
